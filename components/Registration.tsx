...
const Registration: React.FC<RegistrationProps> = ({ onComplete }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'username') {
      // Remove espaços e caracteres especiais do handle, mantém apenas letras, números e underlines
  ...
  const checkHandleAvailability = (handle: string) => {
    ...
    // Simulação de verificação de disponibilidade no servidor
    setTimeout(() => {
      setIsCheckingHandle(false);
      setHandleAvailable(true); // Em um app real, aqui buscaríamos no DB
    }, 600);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newUser: User = {
      ...
      avatar: `https://picsum.photos/seed/${formData.username}/200/200`,
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-6 overflow-y-auto py-12">
      <div className="w-full max-w-md bg-zinc-900 rounded-[3rem] border border-zinc-800 p-8 md:p-10 shadow-3xl space-y-8 relative overflow-hidden">
        ...
        <div className="text-center space-y-3 relative">
          <div className="w-16 h-16 bg-blue-600 rounded-2xl mx-auto flex items-center justify-center shadow-2xl shadow-blue-600/20 mb-2 rotate-3">
            <span className="text-3xl font-black italic tracking-tighter text-white">C</span>
          </div>
          <h1 className="text-3xl font-black italic tracking-tighter text-white">CADASTRO</h1>
